...
bool NullOperationElimination::runOnBasicBlock(BasicBlock &B) {
    ...
    for (auto i = B.begin(); i != B.end();) {
        ...
        if (const auto *andi = dynamic_cast<const AndI *>(&*i)) {
            if (andi->width() == andi->dst()->width()) {
                if (&*andi->dst() == &*andi->src1()) {
                    if (const auto *imm = dynamic_cast<const IntegerImmediate *>(&*andi->src2())) {
                        if (imm->value() == -1) {
                            B.remove(i++);
                            changed = true;
                            continue;
        ...
        if (const auto *ori = dynamic_cast<const OrI *>(&*i)) {
            if (ori->width() == ori->dst()->width()) {
                if (&*ori->dst() == &*ori->src1()) {
                    if (const auto *imm = dynamic_cast<const IntegerImmediate *>(&*ori->src2())) {
                        if (imm->value() == 0) {
                            B.remove(i++);
                            changed = true;
                            continue;
        ...
        if (const auto *xori = dynamic_cast<const XorI *>(&*i)) {
            if (xori->width() == xori->dst()->width()) {
                if (&*xori->dst() == &*xori->src1()) {
                    if (const auto *imm = dynamic_cast<const IntegerImmediate *>(&*xori->src2())) {
                        if (imm->value() == 0) {
                            B.remove(i++);
                            changed = true;
                            continue;
        ...
        if (const auto *shli = dynamic_cast<const SHLI *>(&*i)) {
            if (shli->width() == shli->dst()->width()) {
                if (&*shli->dst() == &*shli->src1()) {
                    if (const auto *imm = dynamic_cast<const IntegerImmediate *>(&*shli->src2())) {
                        if (imm->value() == 0) {
                            B.remove(i++);
                            changed = true;
                            continue;
        ...
        if (const auto *shrli = dynamic_cast<const SHRLI *>(&*i)) {
            if (shrli->width() == shrli->dst()->width()) {
                if (&*shrli->dst() == &*shrli->src1()) {
                    if (const auto *imm = dynamic_cast<const IntegerImmediate *>(&*shrli->src2())) {
                        if (imm->value() == 0) {
                            B.remove(i++);
                            changed = true;
                            continue;
        ...
        if (const auto *shrai = dynamic_cast<const SHRAI *>(&*i)) {
            if (shrai->width() == shrai->dst()->width()) {
                if (&*shrai->dst() == &*shrai->src1()) {
                    if (const auto *imm = dynamic_cast<const IntegerImmediate *>(&*shrai->src2())) {
                        if (imm->value() == 0) {
                            B.remove(i++);
                            changed = true;
                            continue;
